...
static void gnlMenu_Desktop4CBS(void);
...
stu_mode_menu generalModeMenu[GNL_MENU_SUM] = // Define struct array and initialize
	{
		...
		{GNL_MENU_DESKTOP4, DESKTOP_MENU_POS, "Desktop4", gnlMenu_Desktop4CBS, SCREEN_CMD_RESET, 0, 0xFF, 0, 0, 0, 0},
...
static void gnlMenu_Desktop1CBS(void)
{
	...
	case 1:
		...
		display_6_8_number(40, 1, (float)Get_Encoder_Count(&motor_left_encoder)); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
		display_6_8_number(85, 1, (float)Get_Encoder_Count(&motor_right_encoder));
...
static void gnlMenu_Desktop3CBS(void)
{
	...
	case 1:
		...
		display_6_8_number(40, 1, point_A[0]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
...
// Desktop4 - VOFA debug menu
static void gnlMenu_Desktop4CBS(void)
{
	unsigned char keys;
	static uint8_t vofa_display_state = 1;
	if (pModeMenu->refreshScreenCmd == SCREEN_CMD_RESET) // Enter when refresh state
	{
	...
	switch (vofa_display_state)
	{
	case 1:
		vofa_display_state++;
		LCD_clear_L(0, 1);
		display_6_8_string(0, 1, "mL.tar:");
		display_6_8_number(50, 1, motorL.target);
		display_6_8_string(0, 2, "mL.act:");
		display_6_8_number(50, 2, (float)Get_Encoder_Count(&motor_left_encoder));
		break;
	case 2:
		vofa_display_state++;
		LCD_clear_L(0, 3);
		display_6_8_string(0, 3, "mL.kp:");
		display_6_8_number(40, 3, motorL.p);
		display_6_8_string(70, 3, "ki:");
		display_6_8_number(100, 3, motorL.i);
		display_6_8_string(0, 4, "kd:");
		display_6_8_number(40, 4, motorL.d);
		break;
	case 3:
		vofa_display_state++;
		LCD_clear_L(0, 5);
		display_6_8_string(0, 5, "v_l:");
		display_6_8_number(30, 5, v_target_l);
		display_6_8_string(70, 5, "v_r:");
		display_6_8_number(100, 5, v_target_r);
		break;
	case 4:
		vofa_display_state++;
		LCD_clear_L(0, 6);
		display_6_8_string(0, 6, "mR.tar:");
		display_6_8_number(50, 6, motorR.target);
		display_6_8_string(0, 7, "VKp_l:");
		display_6_8_number(40, 7, motorL.p);
		break;
	default:
		vofa_display_state = 1;
	}
	if (pModeMenu->keyVal != 0xff) // Key triggered
	{
		...
		{
		case KEY5_CLICK_RELEASE:
			pModeMenu = &settingModeMenu1[0];
			pModeMenu->refreshScreenCmd = SCREEN_CMD_RESET;
			break;
		case KEY3_CLICK_RELEASE: // Left key
			pModeMenu = &generalModeMenu[2];
			pModeMenu->refreshScreenCmd = SCREEN_CMD_RESET;
			break;
		case KEY4_CLICK_RELEASE: // Right key
			pModeMenu = &generalModeMenu[0];
			pModeMenu->refreshScreenCmd = SCREEN_CMD_RESET;
			break;
		}
	}
...
static void stg2Menu_Para5CBS(void)
{
	...
	display_6_8_number(40, 2, point_A[0]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
...
static void stg3Menu_Para2CBS(void)
{
	...
	//			display_6_8_string(0,2,"A:");			  //cm/s
	display_6_8_number(40, 2, point_A[1]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para3CBS(void)
{
	...
	display_6_8_number(40, 2, point_B[0]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para4CBS(void)
{
	...
	display_6_8_number(40, 2, point_B[1]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para5CBS(void)
{
	...
	display_6_8_number(40, 2, point_C[0]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para6CBS(void)
{
	...
	display_6_8_number(40, 2, point_C[1]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para7CBS(void)
{
	...
	display_6_8_number(40, 2, point_D[0]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
static void stg3Menu_Para8CBS(void)
{
	...
	display_6_8_number(40, 2, point_D[1]); // (float)Get_Encoder_Count(&motor_left_encoder) EncoderA
	//			display_6_8_number(80,1,point_A[1]);
...
void Vofa_pid_tuning(void)
{
	// ��Ļ��ʾ�˶ȺͲ����ٶ����ڵ���

	// VOFA������
	if (vofa_has_param_update())
	{
		uint8_t id = vofa_get_param_id();
		float val = vofa_get_param_value(id);
		switch (id)
		{
		case 1: // �ٶȻ�Kp
			motorL.p = val;
			// motor_right.speed_ctrl.kp = val;
			break;
		case 2: // �ٶȻ�Ki
			motorL.i = val;
			// motor_right.speed_ctrl.ki = val;
			break;
		case 3: // �ٶȻ�Kd
			motorL.d = val;
			// motor_right.speed_ctrl.kd = val;
			break;
		case 4: // Ŀ�ȶ�
			motorL.target = val;
			motorR.target = val;
		...
		// u1_printf("ID:%d Val:%0.3f", id, val);
	}
	// JustFloatЭ�鷢��Ŀ�ȶȺͲ����ٶ�
	static uint8_t send_time = 0;
	if (++send_time >= 10) // Լ10ms����һ��
	{
		send_time = 0;
		uint8_t i;
		float vofa_buf[2];
		vofa_buf[0] = motorL.target;
		vofa_buf[1] = (float)Get_Encoder_Count(&motor_left_encoder);
		// JustFloatЭ�鷢��
		uart1_send_char(0x00); // frame header
		for (i = 0; i < 2; i++)
		{
