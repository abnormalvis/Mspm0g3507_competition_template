...
/* ---- Compatibility stubs for legacy menu globals ----
 * AT24C02 EEPROM is not present in this template; all tunables are RAM-only
 * and lost on reboot. Re-add a persistent storage backend if needed. */
typedef union { float value; unsigned char bytes[4]; } FloatBytes;
static FloatBytes data_test;

typedef struct { unsigned char Start_Car; unsigned char Start_duty_1;
                 unsigned char task_start; unsigned char beep_on; } AppFlag;
typedef struct { unsigned char Send_Step; unsigned char Send2_Step; } AppParam;
typedef struct { float left_motor_speed_cmps; float right_motor_speed_cmps; } AppSmartcarImu;
static AppFlag        Flag;
...
static float sleep_time = 0.0f, yaw_track_kp = 0.0f, yaw_track_kd = 0.0f;
static float v_target_l = 0.0f, v_target_r = 0.0f, speed_setup = 0.0f;
static float distance_inter = 0.0f;

float test_eepro = 0.0f;

extern volatile uint8_t gray_sample_req;

static inline void Storage_WriteFloatNum(unsigned int addr, FloatBytes v) { (void)addr; (void)v; }
static inline void Flag_Init(void) { Flag.task_start = 0; Flag.beep_on = 0; Flag.Start_duty_1 = 0; }
	...
	if(pModeMenu->keyVal != 0xff)
	{
		...
		switch(keys)
		{
			...
			case KEY5_CLICK_RELEASE: //Center key short press
				/* Hand off to chess duty: it owns its own key dispatch via
				 * duty_chess_on_key, registered while running. */
				hal_KeyScanCBSRegister(duty_chess_on_key);
				duty_chess_init();
				duty_chess_run();
				hal_KeyScanCBSRegister(KeyEventHandle);
				pModeMenu->refreshScreenCmd = SCREEN_CMD_RESET;
