...
void DS2024_duty1(void)//任务状态机
{
	
		switch (Param.Send1_Step)//一步一步，不会重复
		{
			case 0:
						Flag.Start_duty_1 = 1;
...
void DS2024_duty2(void)//题目2状态机
{
		switch (Param.Send2_Step)//一步一步，不会重复
		{
			case 0:
				...
				break;
			case 6:			
						Flag.Start_duty2_7 = 1;// C -> D 30s  等待200ms
			...
			case 2:			
					...
					break;						
			case 3:			
						Flag.Start_duty2_4 = 1;// C -> D 30s  智能循迹
					...
					break;
			case 5:			
						Flag.Start_duty2_6 = 1;// C -> D 30s  等待200ms
					...
					break;						
			case 4:			
						Flag.Start_duty2_5 = 1;// D -> A 30s 循迹
...
//偏航角角度环  使用jy62
float yaw_feedback = 0,yaw_error = 0,yaw = 0,target_yaw = 0,yaw_out[2]={0,0};//角度环 （和速度位置串级Pid一致）
...
float position_error,position_feedback,position_output;//位置误差 位置速度 反馈位置
...
float pos_auto_track(float L)//脉冲速度位置
{
	...
	{	
		...
		position_output = pos_track_kp*err_l+ position_kd*(err_l-err_last_l);//位置环输出
...
//		if(L >pos_boudary)
//			position_output = Xianfu_float(position_output,pos_out_limH);//50是目标速度 对位置输出进行限幅
//		else
		
		if(task_num == 4)
				position_output = Xianfu_float(position_output,pos_out_limH);//50是目标速度 对位置输出进行限幅
		else
				position_output = Xianfu_float(position_output,30);//50是目标速度 对位置输出进行限幅
	}
...
{
	switch (Param.Send3_Step)//一步一步，不会重复
		{
			case -1://自动循迹前调整车位
